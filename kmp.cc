...
void KMP::preProcess(const char* pattern, int* lps, int M) {
    //preprocessing
    int len = 0;
...
int KMP::search(std::string* fileData, std::string* word) {
// void KMP(const char* pat, const char* txt) {
    const char* pat = word->c_str();
    const char* txt = fileData->c_str();
    int M = strlen(pat);
    int N = strlen(txt);
    int lps[M];
